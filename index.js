  ...
  return `
    <li class='js-item-element' data-item-id='${item.id}'>
      ...
      <div class='shopping-item-controls'>
        ...
        <button class='shopping-item-save js-item-save' default>
...
const handleEditItemClicked = function(){
  $('.js-shopping-list').on('click', '.js-item-edit', event => {
    ...
    const idEdit = getItemIdFromElement(event.currentTarget);
    let changeindex = store.items.findIndex(item => item.id === idEdit);
    store.items[changeindex].edit = true;  
    $('.js-shopping-list').on('click', '.js-item-save', event => {  
      event.preventDefault();
      const idsave = getItemIdFromElement(event.currentTarget);
      let saveindex=store.items.findIndex(item => item.id === idsave); 
   
    if(idsave===idEdit && saveindex === changeindex){
      let newName = $('.edittext').val();
     store.items[saveindex].name = newName;
      render();
      $('.edittext').val('');
...
/*const saveEditItemClicked = function(){
  $('.js-shopping-list').on('click', '.js-item-save', event => {  
    event.preventDefault();
    const idsave = getItemIdFromElement(event.currentTarget);
    let saveindex = store.items.findIndex(item => item.id === idsave);     
